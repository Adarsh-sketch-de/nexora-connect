...
async function bindGlobal(){
  ...
  if(user){
    ...
    const callModal=document.createElement('div');callModal.className='call-request-modal';callModal.hidden=true;callModal.innerHTML=`<div class="call-request-card"><img class="call-request-avatar"><div class="call-request-type"></div><h2 class="call-request-name"></h2><p class="muted call-request-text"></p><div class="call-request-actions"><button type="button" class="danger call-decline">Decline</button><button type="button" class="primary call-accept">Accept</button></div></div>`;document.body.appendChild(callModal);
    const closeCallModal=()=>{callModal.hidden=true;activeIncomingCall=null};
    globalSocket.on('incoming-call',d=>{activeIncomingCall=d;callModal.querySelector('.call-request-avatar').src=d.callerAvatar||avatar({username:d.callerName});callModal.querySelector('.call-request-type').textContent=d.mode==='audio'?'📞 Incoming audio call':'📹 Incoming video call';callModal.querySelector('.call-request-name').textContent=d.callerName||'Connect user';callModal.querySelector('.call-request-text').textContent=`${d.callerName||'Someone'} wants to start a ${d.mode==='audio'?'audio':'video'} call with you.`;callModal.hidden=false;playAlert();});
    callModal.querySelector('.call-decline').onclick=()=>{if(activeIncomingCall)globalSocket.emit('call-response',{callId:activeIncomingCall.callId,accepted:false});closeCallModal()};
    callModal.querySelector('.call-accept').onclick=()=>{if(!activeIncomingCall)return;globalSocket.emit('call-response',{callId:activeIncomingCall.callId,accepted:true});};
    globalSocket.on('call-accepted',d=>{if(activeIncomingCall?.callId===d.callId)closeCallModal();location.href=`/meeting?room=${encodeURIComponent(d.room)}&conversation=${encodeURIComponent(d.conversationId)}&mode=${encodeURIComponent(d.mode)}`;});
    globalSocket.on('call-cancelled',d=>{if(activeIncomingCall?.callId===d.callId)closeCallModal();});
...
async function initChat(){
  ...
  async function openConversation(id){
    ...
    if(meetingBtn){meetingBtn.disabled=false;meetingBtn.textContent=convo?.type==='group'?'📹 Group video':'📹 Video call'}if(audioBtn){audioBtn.disabled=false;audioBtn.textContent=convo?.type==='group'?'📞 Group audio':'📞 Audio call'}if(moreBtn)moreBtn.disabled=false;if(groupInfoBtn)groupInfoBtn.hidden=convo?.type!=='group'
  ...
  const startRequestedCall=mode=>{if(!current)return;const gs=window.connectGlobalSocket;if(!gs)return alert('Call service is not connected yet.');const label=mode==='audio'?'audio':'video';const overlay=document.createElement('div');overlay.className='call-request-modal outgoing-call-modal';overlay.innerHTML=`<div class="call-request-card"><div class="call-request-type">${mode==='audio'?'📞':'📹'} Calling…</div><h2>${escapeHtml(title?.textContent||'Connect user')}</h2><p class="muted">Waiting for the other participant to accept the ${label} call.</p><button type="button" class="danger cancel-outgoing-call">Cancel call</button></div>`;document.body.appendChild(overlay);let callId=null;const cleanup=()=>overlay.remove();overlay.querySelector('.cancel-outgoing-call').onclick=()=>{if(callId)gs.emit('cancel-call',{callId});cleanup()};gs.emit('call-request',{conversationId:current,mode},r=>{if(!r?.ok){cleanup();return alert(r?.error||'Could not start the call request.')}callId=r.callId});const accepted=d=>{if(callId&&d.callId!==callId)return;cleanup();gs.off('call-accepted',accepted);gs.off('call-declined',declined);gs.off('call-timeout',timed);location.href=`/meeting?room=${encodeURIComponent(d.room)}&conversation=${encodeURIComponent(d.conversationId)}&mode=${encodeURIComponent(d.mode)}`};const declined=d=>{if(callId&&d.callId!==callId)return;cleanup();gs.off('call-accepted',accepted);gs.off('call-declined',declined);gs.off('call-timeout',timed);alert(`${d.by||'The participant'} declined the call.`)};const timed=d=>{if(callId&&d.callId!==callId)return;cleanup();gs.off('call-accepted',accepted);gs.off('call-declined',declined);gs.off('call-timeout',timed);alert('No answer. The call request expired.')};gs.on('call-accepted',accepted);gs.on('call-declined',declined);gs.on('call-timeout',timed)};if(meetingBtn)meetingBtn.onclick=()=>startRequestedCall('video');if(audioBtn)audioBtn.onclick=()=>startRequestedCall('audio');if(mobileAudioBtn)mobileAudioBtn.onclick=()=>audioBtn?.click();if(mobileVideoBtn)mobileVideoBtn.onclick=()=>meetingBtn?.click();if(moreBtn&&moreMenu){moreBtn.onclick=()=>{const open=moreMenu.classList.toggle('open');moreBtn.setAttribute('aria-expanded',String(open))};document.addEventListener('click',e=>{if(!e.target.closest('.chat-actions')){moreMenu.classList.remove('open');moreBtn.setAttribute('aria-expanded','false')}})}
  ...
  async function openGroupInfo(){if(!current)return;try{const j=await api(`/api/conversations/${current}/group-info`);groupInfoTitle.textContent=j.conversation.title||'Group info';groupInfoCount.textContent=`${j.members.length} members`;groupInfoMembers.innerHTML=j.members.map(m=>`<div class="group-info-member"><img class="avatar" src="${avatar(m)}"><div class="member-copy"><b>${escapeHtml(m.username)}${Number(m.id)===Number(user.id)?' (You)':''}</b><small>${escapeHtml(m.role==='admin'?'Group admin':m.email||'Member')}</small></div>${j.isAdmin&&Number(m.id)!==Number(user.id)?`<button class="danger" data-remove-group-member="${m.id}">Remove</button>`:''}</div>`).join('');qsa('[data-remove-group-member]',groupInfoMembers).forEach(b=>b.onclick=async()=>{if(!confirm('Remove this member from the group?'))return;await api(`/api/conversations/${current}/members/${b.dataset.removeGroupMember}`,{method:'DELETE'});await openGroupInfo();await loadConvos()});const friends=(await api('/api/friends')).friends||[],memberIds=new Set(j.members.map(m=>Number(m.id))),available=friends.filter(f=>!memberIds.has(Number(f.id)));groupAddSection.hidden=!j.isAdmin;groupAddSelect.innerHTML=available.map(f=>`<option value="${f.id}">${escapeHtml(f.username)}</option>`).join('')||'<option value="">No connections available</option>';groupAddBtn.disabled=!available.length;groupInfoModal.hidden=false;document.body.classList.add('modal-open')}catch(e){alert(e.message)}}
  groupInfoBtn?.addEventListener('click',openGroupInfo);closeGroupInfo?.addEventListener('click',()=>{groupInfoModal.hidden=true;document.body.classList.remove('modal-open')});groupInfoModal?.addEventListener('click',e=>{if(e.target===groupInfoModal){groupInfoModal.hidden=true;document.body.classList.remove('modal-open')}});groupAddBtn?.addEventListener('click',async()=>{const uid=Number(groupAddSelect.value);if(!uid)return;try{await api(`/api/conversations/${current}/members`,{method:'POST',body:JSON.stringify({userId:uid})});await openGroupInfo();await loadConvos()}catch(e){alert(e.message)}});leaveGroupBtn?.addEventListener('click',async()=>{if(!confirm('Leave this group?'))return;try{await api(`/api/conversations/${current}/leave`,{method:'POST'});groupInfoModal.hidden=true;document.body.classList.remove('modal-open');current=null;document.body.classList.remove('chat-conversation-open');messages.innerHTML='';title.textContent='Select a conversation';membersEl.textContent='';await loadConvos()}catch(e){alert(e.message)}});
...
async function initMeeting(){
  ...
  async function ensureMedia(){
    ...
    else if(callMode==='video'&&!stream.getVideoTracks().length)status.textContent='Microphone ready. No camera is available, but audio, transcription and screen sharing can still work.';
    return stream
  }
  function currentAudioTrack(){return stream?.getAudioTracks()[0]||null}
  ...
  async function stopScreenShare(){
    ...
    local.srcObject=stream||null;screenBtn.textContent='Share screen';if(screenStatus)screenStatus.textContent=stream?.getVideoTracks()[0]?'Screen sharing stopped. Camera restored.':'Screen sharing stopped.'
  }
  screenBtn.onclick=async()=>{
    ...
    try{
      ...
      const track=screenStream.getVideoTracks()[0];if(!track)throw new Error('No screen was selected.');
      for(const pc of peers.values()){const sender=senderForKind(pc,'video');if(sender)await sender.replaceTrack(track)}
      local.srcObject=screenStream;screenBtn.textContent='Stop sharing';if(screenStatus)screenStatus.textContent='You are sharing your screen with the meeting.';track.onended=()=>stopScreenShare();
    }catch(e){if(screenStream){screenStream.getTracks().forEach(t=>t.stop());screenStream=null}if(screenStatus)screenStatus.textContent=e.name==='NotAllowedError'?'Screen sharing was cancelled or not allowed.':e.message}
  };
  ...
  async function loadSavedTranscript(){if(!room)return;try{const j=await api(`/api/meetings/${encodeURIComponent(room)}/transcript`);transcriptEl.innerHTML=j.transcript.length?'':'<p class="muted">No saved transcript yet.</p>';j.transcript.forEach(x=>addTranscript({...x,final:true,speaker:x.speaker_name}));if(j.transcript.length)supportEl.textContent=`${j.transcript.length} saved transcript line(s) loaded.`}catch(e){supportEl.textContent=e.message}}
  const SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;
  let aiTranscription=false;
  try{const cap=await api('/api/transcription/status');aiTranscription=!!cap.configured;if(aiTranscription)supportEl.textContent=`Free local transcription is ready (${cap.model||'Whisper'}). English, Hindi and Marathi are supported.`;else if(SpeechRecognition)supportEl.textContent='Local Whisper is not running; using browser speech recognition as a fallback.';else{supportEl.textContent='Start the free local transcription service on the Connect PC.';transcriptionBtn.disabled=true}}catch(e){supportEl.textContent='Could not check transcription availability.'}
  function blobToDataURL(blob){return new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(blob)})}
  function preferredAudioMime(){const types=['audio/webm;codecs=opus','audio/webm','audio/mp4'];return types.find(t=>window.MediaRecorder?.isTypeSupported?.(t))||''}
  async function recordNextSegment(){
    if(!transcribing||!aiTranscription)return;
    await ensureMedia();const track=await ensureAudioTrack();if(!track){stopTranscription();return}
    const audioStream=new MediaStream([track]);const mime=preferredAudioMime();const opts=mime?{mimeType:mime}:undefined;let chunks=[];
    try{segmentRecorder=new MediaRecorder(audioStream,opts)}catch(e){supportEl.textContent='Could not start audio capture for transcription: '+e.message;stopTranscription();return}
    segmentRecorder.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data)};
    segmentRecorder.onstop=async()=>{if(!chunks.length){if(transcribing)setTimeout(recordNextSegment,150);return}const rawType=segmentRecorder.mimeType||mime||'audio/webm',baseType=String(rawType).split(';')[0]||'audio/webm';const blob=new Blob(chunks,{type:baseType});if(blob.size<2500){supportEl.textContent='Listening… speak for a few seconds.';if(transcribing)setTimeout(recordNextSegment,180);return}try{const audioData=await blobToDataURL(blob);await api('/api/transcription/chunk',{method:'POST',body:JSON.stringify({room,conversationId,language:languageSelect.value,mimeType:baseType,audioData})})}catch(e){supportEl.textContent='Local transcription unavailable; switching to browser live transcription.';aiTranscription=false;if(SpeechRecognition&&transcribing){try{startBrowserRecognition();return}catch{}}else supportEl.textContent='Transcription service: '+e.message}if(transcribing&&aiTranscription)setTimeout(recordNextSegment,180)};
    segmentRecorder.start();segmentTimer=setTimeout(()=>{if(segmentRecorder?.state==='recording')segmentRecorder.stop()},8000)
  }
  function startBrowserRecognition(){manualStop=false;browserRecognition=new SpeechRecognition();browserRecognition.lang=languageSelect.value;browserRecognition.continuous=true;browserRecognition.interimResults=true;browserRecognition.maxAlternatives=1;browserRecognition.onresult=e=>{for(let i=e.resultIndex;i<e.results.length;i++){const text=e.results[i][0]?.transcript?.trim();if(text)socket.emit('meeting-transcript',{room,text,final:e.results[i].isFinal,language:browserRecognition.lang,conversationId})}};browserRecognition.onerror=e=>{supportEl.textContent=`Browser transcription error: ${e.error}`};browserRecognition.onend=()=>{if(transcribing&&!manualStop){try{browserRecognition.start()}catch{}}};browserRecognition.start()}
  function stopTranscription(announce=true){manualStop=true;transcribing=false;if(segmentTimer)clearTimeout(segmentTimer);segmentTimer=null;if(segmentRecorder?.state==='recording')try{segmentRecorder.stop()}catch{}segmentRecorder=null;if(browserRecognition)try{browserRecognition.stop()}catch{}browserRecognition=null;transcriptionBtn.textContent='Start transcription';setIndicator(false);if(announce&&room)socket.emit('meeting-transcription-status',{room,active:false,language:languageSelect.value})}
  async function startTranscription(){if(!room)return alert('Join the meeting room first.');await ensureMedia();if(!(await ensureAudioTrack()))return;manualStop=false;transcribing=true;transcriptionBtn.textContent='Stop transcription';setIndicator(true,'On • '+languageSelect.options[languageSelect.selectedIndex].text);socket.emit('meeting-transcription-status',{room,active:true,language:languageSelect.value});if(aiTranscription)recordNextSegment();else if(SpeechRecognition)startBrowserRecognition();else{supportEl.textContent='Transcription is not configured.';stopTranscription()}}
  transcriptionBtn.onclick=()=>transcribing?stopTranscription():startTranscription();
  languageSelect.onchange=()=>{if(transcribing){stopTranscription(false);setTimeout(startTranscription,300)}};
  clearTranscriptBtn.onclick=()=>{transcriptEl.innerHTML='<p class="muted">Transcript cleared on this device. Saved transcript remains available.</p>'};
  socket.on('meeting-transcript',addTranscript);
  socket.on('meeting-transcription-status',d=>{if(!transcribing)setIndicator(!!d.active,d.active?`On • ${d.username}`:'Off');supportEl.textContent=d.active?`${d.username} enabled live transcription (${d.language||'selected language'}).`:`${d.username} stopped live transcription.`});
  ...
  summaryBtn.onclick=async()=>{if(!room)return alert('Join a meeting room first.');summaryBtn.disabled=true;summaryStatus.textContent='Generating meeting summary from the saved transcript…';try{const j=await api(`/api/meetings/${encodeURIComponent(room)}/summary`,{method:'POST',body:JSON.stringify({language:summaryLanguage.value})});showMeetingSummary(j.summary);summaryStatus.textContent=j.ai?'AI summary generated and saved.':'Summary generated and saved. Add OPENAI_API_KEY for higher-quality multilingual summaries.'}catch(e){summaryStatus.textContent=e.message}finally{summaryBtn.disabled=false}};
  loadTranscriptBtn.onclick=()=>room?loadSavedTranscript():alert('Join a meeting room first.');
  deleteTranscriptBtn.onclick=async()=>{if(!room)return alert('Join a meeting room first.');if(!confirm('Delete the saved transcript for this meeting? Existing summaries will remain saved.'))return;try{await api(`/api/meetings/${encodeURIComponent(room)}/transcript`,{method:'DELETE'});transcriptEl.innerHTML='<p class="muted">Saved transcript deleted.</p>';summaryStatus.textContent='Saved transcript deleted.'}catch(e){summaryStatus.textContent=e.message}};
